...
**Дневной прогон стоит в кроне Victory: `0 4 * * *` UTC = 09:00 Екатеринбург**
(`CRON_SCHEDULE_2026-08-16`, сдвинут с `0 2` 2026-08-24). Victory живёт в UTC, поэтому 09:00 Екб —
это 04:00 в crontab.

⚠️ **Окно нельзя двигать раньше 04:00 UTC.** Внешний загрузчик пишет
`raw_data.yandex_direct_report_rows` с 04:30 до ~06:42 МСК (`system.query_log`: ~185 тяжёлых
INSERT, ~27 млн строк, потом почасовые доливки по ~121k). Прогон в 02:00 UTC (05:00 МСК) попадал
в середину загрузки и брал ~30% вчерашнего расхода: в `big_analytics_full` за 2026-08-23 было
6 524 строки / 2 399 534 ₽ против 21 014 / 7 923 881 ₽ за 08-22, при 120 857 строках /
6 536 187 ₽ в raw. Чинилось только следующим прогоном, `verify_big_analytics` это не ловит —
проверки на полноту последнего дня в нём нет.

```
0 4 * * * cd ~/big_analytics_v6_ch && BA6_POWERBI_REFRESH=1 \
