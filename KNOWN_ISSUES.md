...
После деплоя на Victory `data_check/compare/run.py` дошёл до чисел по прогону `cfecd7759368`.

**Текущий статус.** FIXED как tooling-баг. Сам gate остаётся `FAIL`, но теперь это реальный
v5↔v6 data parity FAIL: расход +0.36%, обращения −1.99%, заявки −0.81%, продажи +0.62%.
Артефакты: `logs/compare_v5_v6_copyfilter_20260819.{txt,json}`.
...
удалены, чтобы старые compatibility-view нельзя было пересоздать ручным вызовом.
