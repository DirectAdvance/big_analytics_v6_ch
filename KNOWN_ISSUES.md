...
### ⚪ #29 — RETIRED: спец-дедуп «Лидер» crmf→mauto убран из заявочной оси

**Суть.** До 2026-08-19 v6 исключал из общего CTE
`step3_build_sources/step3.py::_leads_deduped_cte` crmf-копии салона `Лидер`, если тот же `phone`
после 2026-05-29 был в `mauto_excel`. Это было шире фактического поведения v5 на заявочной оси и
давало ожидаемую дельту против BA5.

**Решение 2026-08-19.** Специальный inline-дедуп `Лидер` удалён из BA6 по BA5-паритету заявочной
оси. `step3` больше не строит `lider_mauto_phones` и не применяет предикат
`source_type='crmf_excel' AND salon='Лидер' ...`.

**Осталось отдельно.** Общий raw-флаг `is_copy_for_removal` не равен этому спец-дедупу. Его
фильтрация остаётся отдельным правилом для мест, где она явно задана (`step13`, pixel и другие
raw-leads потребители).
...
### 🟢 #38 — FIXED 2026-08-19: `data_check/compare` снова доходит до числовой сверки star/light fact

**Суть.** Штатный гейт v5↔v6 падал до сравнения данных. Запуск 2026-08-10:
...
Причина была не в данных, а в контракте: `data_check/compare/run.py` ожидал wide-колонки
...
**Решение 2026-08-19.** Compare-contract переведён на star/light fact: `специалист` берётся через
`Dim_Salon`, `"Название crm"` — через `Dim_CRMStatus`, `источник` уже берётся через `Dim_Source`.
После деплоя на Victory `data_check/compare/run.py` дошёл до чисел; последний принятый прогон
для ручной сверки — `ed6bfc6f9c23`.

**Текущий статус.** FIXED как tooling-баг. Сам gate остаётся `FAIL`, но теперь это реальный
v5↔v6 data parity FAIL: расход +0.36%, обращения −1.98%, заявки −0.81%, продажи +0.62%.
...
### 🔴 #45 — лиды разложены по доменам иначе, чем в Adscope: 465 доменов из 620 расходятся при сходящемся итоге

**Суть.** `d.adscope.ru` читает **ту же CRM-базу**, что и v6, поэтому расхождение целиком внутри
нашей доменной привязки. Замер за июль (1–30, `ad_analytics.raw_leads`, `created_date`):

| | доменов | лидов |
|---|---:|---:|
| наши, с непустым `domain` | 672 | 56 511 |
| Adscope, все сайты | 972 | 57 007 |
| | | **Δ = −496 (−0.9 %)** |

Итог сходится, **поштучная раскладка — нет**: на 620 общих доменах у нас 50 994 лида против 56 209,
число лидов совпадает точно лишь на **155 доменах из 620**. Расхождения двусторонние и гасят друг
друга (недобор 275 / перебор 243 по КО), то есть лиды *перетекают между сайтами*, чаще внутри
одного салона:

| домен | салон | наш | Adscope |
|---|---|---:|---:|
| `topcar-kuban.site` | Кубань Драйв | 14 | 237 |
| `kuban-probeg.ru` | Кубань Драйв | 437 | 286 |
| `ufa-autohouse.ru` | — | 464 | 667 |
| `surgut-multiauto.ru` | — | 330 | 532 |

Разница в 52 «лишних» доменах у нас (5 517 лидов) — не сайты, а площадки: `konvert.ru` (2 358),
`victory-pixel.ru` (1 450), `avito.ru` (544), `perform.ru` (486), `drom.ru`, `auto.ru`; Adscope их в
разрезе сайтов не показывает. Обратно — 352 домена / 798 лидов только у Adscope.

**Корень.** 81 824 из 138 335 июльских лидов (**59 %**) приходят с `domain_id IS NULL` и пустым
`domain` — они не попадают ни на какой сайт. У 21 821 из них `utm_source` похож на домен.

**Что проверено и НЕ сработало.** Гипотеза «Adscope берёт домен из `utm_source`» (она же пункт 2
рекомендаций отчёта `ba5_vs_adscope_kval_visits_sales_2026-08` в v5) **не подтвердилась**: наивный
fallback `domain := utm_source` даёт 70 442 лида против 56 233 (**+14 209**), Σ|Δ| по доменам растёт
с 7 589 до 20 937. На топовых расходящихся доменах `utm_source` — это `s:none`, `s:yandex.ru`,
`yandex_aw`, то есть не домен вовсе. Поле, по которому Adscope привязывает лид к сайту, не найдено;
кандидаты на проверку — `fid`, `campaign_id → сайт`, форма заявки.

**Почему важно.** Пока раскладка расходится, **поддоменная** сверка любой метрики воронки с Adscope
недостижима независимо от маппинга статусов. Итоговые (по всем доменам) значения при этом сверяемы:
после фикса reason-метрик КО и добро сходятся в пределах 1.3 %.

**Как воспроизвести сверку.** Логин POST `{ADSCOPE_URL}/login` (`ADSCOPE_LOGIN`/`ADSCOPE_PASSWORD` из
`.secret/.env`), затем GET `/dashboard?month=YYYY-MM`; в HTML — embedded JSON `const D = {...}`,
нужные метрики в `D.salons[].sites[]`: `total_leads`, `quals`, `visits`, `credits`, `approveds`,
`sales`. `D.period.leads_last_update` показывает, по какую дату закрыты лиды.

**Статус.** OPEN. Замер 2026-08-25.
