...
- Результат: строки в `big_analytics_pixel_score` и итоговый канон в витринах:
  `_source_table='pixel'`, `источник='Пиксель'`, `направление='Пиксель'`. Меры остаются
  **дробными долями**, поэтому требование numeric сохраняется. Старое значение
  `Пиксель_атрибуц` выведено из BA6-контракта 2026-08-17.
