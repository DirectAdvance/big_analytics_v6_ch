...
**Логика v6_ch с 2026-08-25:** метрики считаются через `reference_data.crm_status_mapping` по
полному ключу `crm/status/salon/reason`, а не голым списком статусов. `reason` и `salon` матчятся
через `lowerUTF8`, чтобы кириллица вроде `Был в КСО` не выпадала из правил.

**dohod_do_kredita** — категории `credit + approved + sale`, кроме cash-sale статусов без
исключающей причины `был в ксо`.

**dobro** — категории `approved + sale`, кроме cash-sale статусов.
