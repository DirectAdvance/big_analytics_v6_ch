...
3. **⚠️ ОПЕРАЦИОННАЯ ПРОБЛЕМА, не связанная с миграцией:** `raw_data.etl_runs` показывает,
   ...
   **Это отдельная задача, не блокирует наш DDL-этап, но Семёну стоит проверить отдельно —
   ...
   ⚠️ **Стало актуальнее 2026-07-30:** решено использовать `raw_data` как источник вместо
   переноса своего step0/step1 (см. §6 п.4) — значит, эта сломанная загрузка Директа/VK Ads
   теперь напрямую бьёт по свежести данных v6_ch. Починка/владелец этого ETL — приоритет
   ДО того, как v6_ch начнёт читать `raw_data` в проде.
...
4. **✅ Решение по доверию `raw_data` (2026-07-30):** не доверять вслепую (одна сверка уже
   ...
   TODO для Этапа 2 (перед тем как строить на `raw_data`): точечно сверить 3-5 ключевых таблиц
   (`leads_all`, `direct_campaigns`, `crm_status_mapping`, `yandex_direct_report_rows`,
   `metrika_yandex_utm_daily`) построчно/по агрегатам против PG-источника — не полагаться на
   штамп `reconciliation_results.status='passed'` без собственной проверки хотя бы по этим 5.
...
5. **Параллельный режим v5/v6:** работают одновременно на одних данных или v6_ch начнёт
   читать из тех же источников что v5? Нужна изоляция источников или можно шарить? — открыт.
