...
| `_source_table` | text | step3/6/10/11 | Технический маркер: `direct`/`seo`/`crop_targeting`/`tp8`/`tp9`/`tp10`/`calls`/`social_посевы`/`telegram`/`reviews`/`пиксель_атрибуц`. Партиции PBI. |
...
| `номер кампании \| название кампании` | text | step6 | Объединённое поле с emoji-префиксом статуса (🟢/🟡/⚪). |
...
| `источник` | text | step3/6/10/11 | **NEVER NULL** (CANON): `Контекст`/`пиксель`/`пиксель_атрибуц`/`звонки`/`telegram`/`Max`/`VK`/`SEO`/`контекст`. |
| `направление` | text | step3/6/11 | Бизнес-категория: `Контекст`/`SEO`/`SEO Flow`/`посевы`/`пиксель`/`пиксель_атрибуц`/`отзывы` (CANON). |
