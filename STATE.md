...
- Задачи **P6** (визитная ось пересчёт, а не копия), **P7** (VK Ads 90 аккаунтов→4, таблица фидов), **P8** (перенос corrections.py), **P9** (дроп `big_analytics_sources` ломает 5 вьюх), **P12** (паритет веток Директа: каскад, перекраска посевов, `источник` по статусу), **P13** (fail-fast на покрытие статусов marcar).
