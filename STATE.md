...
аккаунтов несут ровно 0.00 ₽. CRM-лиды в v6 больше v5 в каждом месяце 2026, `Купил` совпадает
1-в-1. `perform_leads` (разрыв 10.08) закрыт. Витрина сошлась: заявочная ось Feb–Jul без пикселя
даёт cost +0.36%, приезды −0.08%, продажи −0.03%.
...
5. **PBI-модель не прочёсана после разведения пикселя.** Пиксельная воронка подписана разными
   метками на разных осях (`Пиксель` на заявочной, `Пиксель_атрибуц` на визитной) — визуал или
   мера, режущая `источник='Пиксель_атрибуц'` на заявочной оси, покажет ноль.
...
- **Инстанс ClickHouse — 2 vCPU / 8.33 ГБ**, серверный потолок 7.49 ГБ, запрос ограничен 2 ГБ
  ...
  это потолок ширины.
- **Крона у v6 нет.** step14 и ночной пайплайн — только руками (#42).
