...
⛔ СТАТУС 2026-08-06 — обе ветки НЕ ПРИМЕНЕНЫ и применять их НЕ требуется.
   ...
   справочника — строки справочника для этих пар игнорируются.

   • ВЕТКА A (marcar) — БОЛЬШЕ НЕ НУЖНА. Источник истины для четырёх статусов
     патча Маркара — код; применение A ничего не сломает (значения совпадут), но и
     ничего не изменит. Скрипт оставлен как история правки и как ссылка из
     `KNOWN_ISSUES`/`STATE`, запускать его не нужно.
   • ВЕТКА B (plex) — в коде лежит ВРЕМЕННЫЙ мост (та же пара → 'qualified').
     Появится GRANT — правильнее применить B в справочнике и снять пару
     ('plex', 'Отказ клиента') из CODE_STATUS_CATEGORY; гард
     `check_code_status_categories()` сам напишет в лог, что справочник и код сошлись.
